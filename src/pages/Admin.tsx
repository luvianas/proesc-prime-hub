...
const Admin = () => {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Administração do Portal Prime para escolas.");
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data, error } = await supabase
        ...
        .select("id,school_id,school_name,logo_url,consultant_name,consultant_photo_url,consultant_whatsapp,consultant_calendar_url,theme_color,dashboard_links")
  ...
  }, [isAdmin, selected?.school_id, selected?.id]);
  ...
  const saveField = async (field: keyof SchoolCustomization, value: any) => {
    ...
    setSchools((prev) => prev.map((s) => (s.id === selected.id ? { ...s, [field]: value } : s)));
    toast.success("Alterações salvas");
  };

  const saveDashboardLink = async (key: string, value: string) => {
    if (!selected) return;
    const updated = { ...(selected.dashboard_links || {}), [key]: value } as Record<string, string>;
    await saveField("dashboard_links", updated as any);
  };

  const uploadToBucket = async (file: File, path: string) => {
    const { data, error } = await supabase.storage.from("school-assets").upload(path, file, { upsert: true });
  ...
  const onLogoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const url = await uploadToBucket(file, `schools/${selected.school_id}/logo-${Date.now()}.png`);
  ...
  const onConsultantPhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selected?.school_id) return toast.error("Crie a escola primeiro");
    const file = e.target.files?.[0];
    if (!file) return;
    try {
      const url = await uploadToBucket(file, `schools/${selected.school_id}/consultant-${Date.now()}.png`);
  ...
  const addBanner = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    try {
      const url = await uploadToBucket(file, `schools/${selected.school_id}/banners/${Date.now()}-${file.name}`);
      const { data, error } = await supabase
        .from("school_banners")
        .insert({ school_id: selected.school_id, image_url: url, order_index: (banners[banners.length - 1]?.order_index ?? -1) + 1 })
        .select("id,school_id,image_url,title,link_url,order_index")
        .single();
      if (error) throw error;
  ...
  const seedAdmins = async () => {
    ...
    if (res.error) return toast.error("Falha ao criar admins");
    ...
    return (
      <div className="max-w-3xl mx-auto p-6">
        <Card>
          ...
          <CardContent>Você precisa ser admin para acessar esta página.</CardContent>
        </Card>
      </div>
    );
  }
    ...
    <main className="max-w-6xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={createSchool}>Nova escola</Button>
      ...
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="flex gap-2 flex-wrap">
            {schools.map((s) => (
              <Button key={s.id} variant={selectedId === s.id ? "default" : "outline"} onClick={() => setSelectedId(s.id)}>
                {s.school_name}
              </Button>
            ))}
          </div>
          ...
          <CardContent>
            <Tabs defaultValue="branding">
              ...
              <TabsContent value="branding" className="pt-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="col-span-2">
                    ...
                    <Input defaultValue={selected.school_name} onBlur={(e) => saveField("school_name", e.target.value)} />
                  </div>
                  <div>
                    <Label>Cor do tema (hex)</Label>
                    <Input defaultValue={selected.theme_color ?? "#c41133"} onBlur={(e) => saveField("theme_color", e.target.value)} />
                  </div>
                  <div className="md:col-span-3">
                    <Label>Logo</Label>
                    <div className="flex items-center gap-4">
                      {selected.logo_url && <img src={selected.logo_url} alt="Logo" className="h-12" />}
              ...
              <TabsContent value="consultant" className="pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input defaultValue={selected.consultant_name ?? ""} onBlur={(e) => saveField("consultant_name", e.target.value)} />
                  </div>
                  <div>
                    <Label>WhatsApp do consultor (somente números)</Label>
                    <Input defaultValue={selected.consultant_whatsapp ?? ""} onBlur={(e) => saveField("consultant_whatsapp", e.target.value)} />
                  </div>
                  <div className="md:col-span-2">
                    <Label>URL da agenda (Google Appointments)</Label>
                    <Input defaultValue={selected.consultant_calendar_url ?? ""} onBlur={(e) => saveField("consultant_calendar_url", e.target.value)} />
                  </div>
                  <div className="md:col-span-2">
                    <Label>Foto do consultor</Label>
                    <div className="flex items-center gap-4">
                      {selected.consultant_photo_url && (
                        <img src={selected.consultant_photo_url} alt="Consultor" className="h-16 w-16 rounded-full object-cover" />
                      )}
              ...
              <TabsContent value="dashboards" className="pt-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(["financeiro_url", "tickets_url", "pedagogico_url"] as const).map((key) => (
                    <div key={key}>
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  <Badge variant="secondary">Dica</Badge> Você pode adicionar mais chaves no banco em dashboard_links.
                </div>
              </TabsContent>
                  ...
                  {banners.map((b) => (
                    <Card key={b.id}>
                      <CardContent className="p-4">
                        <img src={b.image_url} alt={b.title ?? "Banner"} className="w-full h-32 object-cover rounded" />
                        <div className="mt-3 space-y-2">
                          <div>
                            <Label>Título</Label>
                            <Input defaultValue={b.title ?? ""} onBlur={(e) => updateBanner(b.id, { title: e.target.value })} />
                          </div>
                          <div>
                            <Label>Link</Label>
                            <Input defaultValue={b.link_url ?? ""} onBlur={(e) => updateBanner(b.id, { link_url: e.target.value })} />
                          </div>
                          <div>
                            <Label>Ordem</Label>
                            <Input type="number" defaultValue={b.order_index} onBlur={(e) => updateBanner(b.id, { order_index: Number(e.target.value) })} />
                          </div>
                          <div className="flex justify-end">
                            <Button variant="destructive" onClick={() => removeBanner(b.id)}>Remover</Button>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
